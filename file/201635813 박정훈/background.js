...
// using mouse click down
    ...
    function object() {
        ...
        // BODY
        draw(gl.TRIANGLE_FAN, head, head_color, [0.0,0.0, 0.0, 10 + count/10]);
        for (count = 0; count < 5; count++) {
            if(second_x!=0){
            slope= second_x/second_y;
            if(count==0){
                draw(gl.TRIANGLE_FAN, head, head_color, [0.0,0.0, 0.0, 10 + count/10]);
            }
            else if(count==1) {
                draw(gl.TRIANGLE_FAN, head, head_color, [second_y, second_x, 0.0, 10 + count/10]);
            }
            else {
                draw(gl.TRIANGLE_FAN, head, head_color, [(second_x-count+1)/(second_x/second_y), second_x-count+1, 0.0, 10 + count/10]);
            }
        }
        else{
            draw(gl.TRIANGLE_FAN, head, head_color, [0.0,-count, 0.0, 10 + count/10]);
        }
            // y=slope * x
            // x=(second_x-count+1)/(second_x/second_y) 
            // second_
